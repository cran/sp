...
{\tt SpatialLines} from scratch. Note that the {\tt Slines} objects have to be given character ID values, and that these values must be unique for {\tt Slines} objects combined in a {\tt SpatialLines} object.
